...
export const shutterStockImages = (searchQuery) => {  
 ...
 const SHUTTERSTOCK_IMAGE_API_ENDPOINT = 'https://api.shutterstock.com/v2/videos/search?query='+searchQuery+'&safe=true&image_type=photo&orientation=horizontal&page=1&per_page=9';
  
  return fetch(SHUTTERSTOCK_IMAGE_API_ENDPOINT, authParameters)
  .then(response => {    
    return response.json();
  })
  .then(json => {      
      return json.data.map(({ id, assets, description }) => ({
        id,
        mediaUrl: assets.preview_jpg.url,
        description
